...
type EmbeddedArray struct {
	ArrayAlias `kosa:"array_alias"`
	Vunique    string
}

type SquashOnNonStructType struct {
	InvalidSquashType int `kosa:",squash"`
}
...
type Tagged struct {
	Extra string `kosa:"bar,what,what"`
	Value string `kosa:"foo"`
}

type Remainder struct {
	A     string
	Extra map[string]interface{} `kosa:",remain"`
}
